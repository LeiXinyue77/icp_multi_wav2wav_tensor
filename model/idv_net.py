...
def croppCenter(tensorToCrop, finalShape):
    org_shape = tf.shape(tensorToCrop)
    diff = org_shape[1] - finalShape[1]  # 计算时间维度差值
    croppBorders = diff // 2
...
class IVD_Net_asym(Model):
    ...
    def call(self, inputs, training=False, mask=None):
        ...
        # (n, 512, 24)
        input_2nd_0 = tf.concat((down_1_0m, down_1_1m, down_1_2m), axis=-1)
        input_2nd_1 = tf.concat((down_1_1m, down_1_2m, down_1_0m), axis=-1)
        ...
        input_3rd_0 = tf.concat((down_2_0m, down_2_1m, down_2_2m), axis=-1)  # (n, 256, 48)
        input_3rd_0 = tf.concat((input_3rd_0, croppCenter(input_2nd_0, input_3rd_0.shape)), axis=-1)  # (n, 256, 72)

        input_3rd_1 = tf.concat((down_2_1m, down_2_2m, down_2_0m), axis=-1)   # (n, 256, 48)
        input_3rd_1 = tf.concat((input_3rd_1, croppCenter(input_2nd_1, input_3rd_1.shape)), axis=-1)  # (n, 256, 72)

        input_3rd_2 = tf.concat((down_2_2m, down_2_0m, down_2_1m), axis=-1)  # (n, 256, 48)
        input_3rd_2 = tf.concat((input_3rd_2, croppCenter(input_2nd_2, input_3rd_2.shape)), axis=-1)   # (n, 256, 72)
        ...
        input_4th_0 = tf.concat((down_3_0m, down_3_1m, down_3_2m), axis=-1)  # (n, 128, 96)
        input_4th_0 = tf.concat((input_4th_0, croppCenter(input_3rd_0, input_4th_0.shape)), axis=-1)   # (n,672,128)

        input_4th_1 = tf.concat((down_3_1m, down_3_2m, down_3_0m), axis=-1)
        input_4th_1 = tf.concat((input_4th_1, croppCenter(input_3rd_1, input_4th_1.shape)), axis=-1)

        input_4th_2 = tf.concat((down_3_2m, down_3_0m, down_3_1m), axis=-1)
        input_4th_2 = tf.concat((input_4th_2, croppCenter(input_3rd_2, input_4th_2.shape)), axis=-1)
        ...
        inputBridge = tf.concat((down_4_0m, down_4_1m, down_4_2m), axis=-1)  # (n, 64, 192)
        inputBridge = tf.concat((inputBridge, croppCenter(input_4th_0, inputBridge.shape)), axis=-1)  # (n, 64, 360)
        bridge = self.bridge(inputBridge, training=training)   # (n, 64, 64)

        # ############################# #
        # ~~~~~~ Decoding path ~~~~~~~  #
        deconv_1 = self.deconv_1(bridge, training=training)   # (n, 128, 64)
        skip_1 = (deconv_1 + down_4_0 + down_4_1 + down_4_2)/4    # (n, 128, 64)  # Residual connection
        up_1 = self.up_1(skip_1, training=training)   # (n, 128, 32)
        deconv_2 = self.deconv_2(up_1, training=training)   # (n, 256, 32)
        skip_2 = (deconv_2 + down_3_0 + down_3_1 + down_3_2)/4   # (n, 256, 32) # Residual connection
        up_2 = self.up_2(skip_2, training=training)    # (n, 256, 16)
        deconv_3 = self.deconv_3(up_2, training=training)   # (n, 512, 16)
        skip_3 = (deconv_3 + down_2_0 + down_2_1 + down_2_2)/4  # (n, 512, 16) # Residual connection
        up_3 = self.up_3(skip_3, training=training)  # (n, 512, 8)
        deconv_4 = self.deconv_4(up_3, training=training)  # (n, 1024, 8)
        skip_4 = (deconv_4 + down_1_0 + down_1_1 + down_1_2)/4  # (n, 1024, 8) # Residual connection
...
if __name__ == "__main__":
    ...
    model = IVD_Net_asym(output_nc=1, ngf=8)
    # model.build(input_shape=(None, 1024, 3))

    test_input = np.random.rand(16, 1024, 3).astype(np.float32)
    test_output = model(test_input)
    print(f"Test input shape: {test_input.shape}")
    print(f"Test output shape: {test_output.shape}")

    x_train = np.random.rand(16, 1024, 3).astype(np.float32)
    y_train = np.random.rand(16, 1024, 3).astype(np.float32)
